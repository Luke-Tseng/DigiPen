...
template <typename KEY_TYPE, typename VALUE_TYPE>
CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::Node(KEY_TYPE k, VALUE_TYPE val, Node *p, int h, int b, Node *l, Node *r) : key(k), value(val), height(h), balance(b),
																													   parent(p),
																													   left(l), right(r)
{
}
...
template <typename KEY_TYPE, typename VALUE_TYPE>
int CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::getHeight()
{
	return height;
}

template <typename KEY_TYPE, typename VALUE_TYPE>
int CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::getBalance()
{
	return balance;
}

template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::setHeight(int h)
{
	height = h;
}

template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::setBalance(int b)
{
	balance = b;
}

template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::findBoth(Node *node)
{
	if (node == NULL)
		return;
	...
	if (node->left)
	{
		lh = node->left->getHeight() + 1;
	}

	if (node->right)
	{
		rh = node->right->getHeight() + 1;
	}

	if (!node->left && !node->right)
		node->setHeight(0);
	else
		node->setHeight(std::max(lh, rh));

	node->setBalance(rh - lh);
}

template <typename KEY_TYPE, typename VALUE_TYPE>
int CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::findHeightAll(Node *node)
{
	if (node == NULL)
		return 0;

	int lh = findHeightAll(node->left);
	int rh = findHeightAll(node->right);
...
template <typename KEY_TYPE, typename VALUE_TYPE>
bool CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::AVLmap_iterator::operator!=(const AVLmap::AVLmap_iterator &rhs)
{
...
template <typename KEY_TYPE, typename VALUE_TYPE>
CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::AVLmap() : pRoot(nullptr), size_(0), nodes(0)
{
}

// Copy constructor
template <typename KEY_TYPE, typename VALUE_TYPE>
CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::AVLmap(const AVLmap &rhs) : pRoot(nullptr), size_(rhs.size_), nodes(0)
{
...
template <typename KEY_TYPE, typename VALUE_TYPE>
CS280::AVLmap<KEY_TYPE, VALUE_TYPE> &CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::operator=(const AVLmap<KEY_TYPE, VALUE_TYPE> &rhs)
{
	if (this != &rhs)
	{
		deleteTree(pRoot);
...
template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::copyTree(Node *rhsNode, Node *&newNode, Node *parent)
{
	if (rhsNode == nullptr)
	{
		newNode = nullptr;
...
VALUE_TYPE &CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::operator[](KEY_TYPE const &key)
{
	...
	else
	{
		...
		while (nodes.size() != 0)
		{
			...
			Node::findBoth(current);
			Node *&node = findNode(current);
			if (node != nullptr)
			{
				if (node->getBalance() > 1 && node->left == nullptr) // right right
				{
					rotateLeft(node);
				}
				else if (node->getBalance() < -1 && node->right == nullptr) // left left
				{
					rotateRight(node);
				}
				...
				{
					if (node->getBalance() > 1 && node->left->getBalance() < 0) // right right
					{
						rotateLeft(node);
					}
					else if (node->getBalance() < -1 && node->left->getBalance() >= 0) // left right
					{
				...
				{
					if (node->getBalance() < -1 && node->right->getBalance() > 0) // left left
					{
						rotateRight(node);
					}
					else if (node->getBalance() > 1 && node->right->getBalance() <= 0) // right left
					{
...
template <typename KEY_TYPE, typename VALUE_TYPE>
typename CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node *&CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::findNode(Node *node)
{
	if (!node->parent)
		return pRoot;
...
template <typename KEY_TYPE, typename VALUE_TYPE>
typename CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node *CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::insert(Node *&tree, Node *parent, KEY_TYPE const &key)
{
	if (tree == nullptr)
	{
...
template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::deleteTree(Node *tree)
{
	if (tree == nullptr)
		return;
...
typename CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::AVLmap_iterator CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::find(KEY_TYPE const &key)
{
	...
	while (node)
	{
		if (key < node->Key())
			...
			return AVLmap<KEY_TYPE, VALUE_TYPE>::AVLmap_iterator(node);
	}
	return end_it;
}
...
template <typename KEY_TYPE, typename VALUE_TYPE>
typename CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node *&CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::Node::findNode(Node *node)
{
...
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::rotateLeft(Node *&node)
{
	...
	Node::findBoth(temp);
	Node::findBoth(copyRoot);
	updateHeight(copyRoot);
}
...
template <typename KEY_TYPE, typename VALUE_TYPE>
void CS280::AVLmap<KEY_TYPE, VALUE_TYPE>::updateHeight(Node *&node)
{
