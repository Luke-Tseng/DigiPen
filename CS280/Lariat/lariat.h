...
public:
    ...
    friend class Lariat;
    template <typename U, int Size2>
    Lariat(const Lariat<U, Size2> &rhs);

    Lariat<T, Size> &operator=(const Lariat<T, Size> &rhs);

    template <typename U, int Size2>
    Lariat<T, Size> &operator=(const Lariat<U, Size2> &rhs);
    // inserts
...
public:
    // Helper functions
    void split(LNode *node, int localIndex);
    std::pair<LNode *, int> findElement(int index);
    void swap(T &a, T &b);
