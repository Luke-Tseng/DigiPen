...
public:
    Clause() {}
    // Clause(std::set<Literal> const &l) : literals(l) {}
    Clause(Literal const &l) { literals.emplace(l); }

    bool const operator<(Clause const &op2);
    bool operator<(Clause const &op2) const;
    std::set<Clause> operator~() const;
    Clause operator|(Clause const &op2) const;

    // Add to pl.h in the Clause class
    bool operator==(Clause const &op2) const
    {
...
public:
    CNF() {}
    CNF(Literal const &l) { clauses.emplace(l); }
    CNF(std::set<Clause> const &c) : clauses(c) {}
