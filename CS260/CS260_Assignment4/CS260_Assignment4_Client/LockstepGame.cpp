...
LockstepGame::LockstepGame(const SOCKET socket, const bool is_host)
	: socket_(socket), is_host_(is_host), is_local_update_sent_(false), turn_number_(0)
{
	...
	if (ioctlsocket(socket_, FIONBIO, &nonblocking) == SOCKET_ERROR &&
		LockstepGame_HandleSocketError("Error setting socket to non-blocking: "))
	{
		return;
	}
...
void LockstepGame::Update()
{
	...
	if (!is_local_update_sent_ && local_player_.has_uncommitted_move)
	{
		...
		if (send(socket_, network_buffer_, static_cast<int>(send_size), 0) == SOCKET_ERROR &&
			LockstepGame_HandleSocketError("Error sending state to remote player: "))
		{
			return;
		}
	...
	const int res = recv(socket_, network_buffer_, kNetworkBufferSize, 0); // replace "0" with recv...
...
{
	std::memset(network_buffer_, 0, kNetworkBufferSize);

	// serialize the turn number, identifying this move
	std::memcpy(network_buffer_, &turn_number_, sizeof(int));

	// serialize the uncommitted position of the local player
	std::memcpy(network_buffer_ + sizeof(int), &local_player_.uncommitted_position.x, sizeof(int));
	std::memcpy(network_buffer_ + sizeof(int) * 2, &local_player_.uncommitted_position.y, sizeof(int));
...
void LockstepGame::DeserializeRemoteState(const size_t bytes_received)
{
	...
	const auto remote_turn_number = *reinterpret_cast<const int *>(network_buffer_); // replace "0" with the deserialized data...
	...
	remote_player_.uncommitted_position.x = *reinterpret_cast<const int *>(network_buffer_ + sizeof(int));
	remote_player_.uncommitted_position.y = *reinterpret_cast<const int *>(network_buffer_ + sizeof(int) * 2);
