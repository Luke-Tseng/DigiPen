...
void OptimisticClientScenarioState::Update()
{
	...
	auto last_direct_local_x = direct_local_control_.GetCurrentX();
	auto last_direct_local_y = direct_local_control_.GetCurrentY();
	...
	auto delta_x = direct_local_x - last_direct_local_x;
	auto delta_y = direct_local_y - last_direct_local_y;
	//TODO: push a new movement record onto the *back* of the history dequed
	local_movement_history_.push_back({ local_frame_, delta_x, delta_y });

	//TODO: while the size of the history deque is greater than kMaxMoveHistorySize, pop entries off the *front* of the deque
	while (local_movement_history_.size() > kMaxMoveHistorySize)
	{
		local_movement_history_.pop_front();
	}
	...
	if (res > 0)
	{
		...
		if (received_frame > remote_frame_)
		{
			...
			auto record_iter = std::find_if(local_movement_history_.begin(), local_movement_history_.end(), [=](LocalMovementRecord record) { return record.frame == last_received_local_frame; });
			
			if (record_iter == local_movement_history_.end())
			{
				std::cerr << "Frame " << last_received_local_frame << " was not found in the local movement history" << std::endl;
			}
			...
			while (record_iter != local_movement_history_.end())
			{
				non_host_x += record_iter->dx;
				non_host_y += record_iter->dy;
				record_iter++;
			}
