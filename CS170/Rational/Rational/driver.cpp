...
void test0()
{
    CS170::Rational r1(1,2);
    std::cout << r1 << std::endl;

    CS170::Rational r2(-1,2);
    std::cout << r2 << std::endl;

    CS170::Rational r3(1,-2);
    std::cout << r3 << std::endl;
}

void test1()
{
    CS170::Rational r1(4,8);
    std::cout << r1 << std::endl;

    CS170::Rational r2(-6,8);
    std::cout << r2 << std::endl;

    CS170::Rational const r3(3,-9);
    std::cout << r3 << std::endl;
}
...
void test3()
{
    ...
    CS170::Rational const r2( r1 );

    CS170::Rational r3(3,-9);
    std::cout << r1 << std::endl;
    std::cout << r2 << std::endl;
    std::cout << r3 << std::endl;
...
void test4()
{
    CS170::Rational const pi_1( 16, 5 );     // approximation of pi (see "Indiana Pi Bill" of 1897)
    CS170::Rational const pi_2( 22, 7 );     // approximation of pi (Ancient Egypt)
    CS170::Rational const pi_3( 355, 113 );  // approximation of pi (Milu approximation by Chiniese astronomer Zu Chongzhi)
...
void test5()
{
    CS170::Rational const r1( -1,2 );
    CS170::Rational const r2( 1,-2 );
    CS170::Rational const r3( 10,-20 );
...
void test6()
{
    CS170::Rational const one_sixth( 1,6 );
    CS170::Rational r; // default = zero (0/1)
...
void test8()
{
    ...
    for( int i=0; i<60; ++ i ) {
        CS170::Rational r( one_sixth );
        r *= i;                     // conversion int -> CS170::Rational in the second argument will be done by compiler by calling CS170::Rational(int)
        std::cout << r << " ";
    }
    std::cout << std::endl;
}
...
{
    CS170::Rational const one_sixth( 1,6 );
    CS170::Rational r( -one_sixth );   // -1/6, requires unary negation operator

    for( int i=0; i<60; ++ i ) {
        CS170::Rational m( one_sixth );
        std::cout << m*i << " ";
...
void test10()
{
    CS170::Rational const one_third( 1,3 );
    int      const two = 2;
    std::cout << one_third - two << std::endl;
    std::cout << one_third + two << std::endl;
    std::cout << one_third / two << std::endl;
}
...
void test11()
{
    ...
    int len = 10;
    CS170::Rational pi; // 0/1 by default
    for ( int i=0; i<len; ++i ) {
        pi += (i%2 ? CS170::Rational(-4) : CS170::Rational(4) ) / (2*i+1);
...
void test12()
{
    ...
    CS170::Rational pi(2);
    for ( int i=1; i<=len; ++i ) {
        pi *= CS170::Rational( 4*i*i,4*i*i-1 );
        std::cout << pi << " appr " << std::setprecision(8) << static_cast<double>( pi.numerator() ) / pi.denominator() << std::endl;
    }
}
