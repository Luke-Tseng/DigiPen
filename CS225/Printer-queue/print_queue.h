...
public:
	...
	PrintQueue(const PrintQueue &rhs);
...
private:
	...
	// all printers in the system are stored herer
...
class EventJobFinished : public TimedCommand
{
	...
	PrintJob job;
public:
	EventJobFinished(double _when, PrintQueue *_pqueue, int _printerID, PrintJob _job);
	EventJobFinished(const EventJobFinished &rhs);
	EventJobFinished &operator=(const EventJobFinished &rhs);
	void Execute();
};
...
public:
	...
	EventNewJobArrived(const EventNewJobArrived &rhs);
	EventNewJobArrived &operator=(const EventNewJobArrived &rhs);
	void Execute();
};
